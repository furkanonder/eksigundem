...
def init_colors() -> bool:
    """Terminal coloring for Windows is written with Windows Console API Functions using the following resource.
    ...
    if sys.platform == "win32":
        try:
            ...
            def get_std_handle() -> WINFUNCTYPE:
                """GetStdHandle retrieves a handle to the specified standard device (standard input, standard output,
                or standard error).
                """
            ...
            def get_console_mode() -> WINFUNCTYPE:
                """GetConsoleMode retrieves the current input mode of a console's input buffer or the current output
